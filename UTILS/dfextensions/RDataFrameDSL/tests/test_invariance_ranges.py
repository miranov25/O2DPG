...
class TestSlidingRangesDSL:
    ...
    def test_INV_R1_SLICE_preserves_order(self, toy_lorentz_file):
        ...
        Phase 13.6.C: Fixed - track_idx is auto-generated by flatten,
        not fetched from ROOT. Removed from column request.
        """
        try:
            import ROOT
            from RDataFrameDSL import DSLCompiler
        ...
        try:
            ...
            # to_pandas adds track_idx automatically for 1D columns
            # Don't request track_idx - it's synthetic, not from ROOT
    ...
    def test_INV_R2_SLICE_2d_slicing(self, alice_rdf):
        """
        Test DSL slicing on 2D column: cluster_Q[:3]
        
        Note: cluster_Q[:3] slices the OUTER dimension (tracks),
        giving the first 3 tracks with all their clusters.
        
        Type: B (DSL end-to-end)
        Priority: P1
        
        Phase 13.6.C: Fixed - corrected slice semantics (outer dim = tracks).
        """
        try:
            from RDataFrameDSL import DSLCompiler
        except ImportError:
        ...
        # Schema for ALICE data - cluster_Q is 2D: RVec(tracks) of RVec(clusters)
        schema = {
            'event_id': 'long',
        ...
        try:
            # Slice 2D column - first 3 TRACKS (outer dimension)
            dsl.define("first_3_tracks_Q", "cluster_Q[:3]")
            
            # to_pandas auto-adds track_idx and cluster_idx for 2D columns
            df = dsl.to_pandas(alice_rdf, ['first_3_tracks_Q', 'event_id'])
    ...
    def test_INV_R5_SUM_dsl_sum_preservation(self, toy_lorentz_file):
        ...
        Sum of sliced values should be <= full sum.
        
        Type: B (DSL end-to-end)
        Priority: P1
        
        Phase 13.6.C: Fixed - use separate to_pandas() calls because
        full and sliced columns have different per-event lengths.
        Mixing different-length 1D columns is undefined in TTree::Draw semantics.
        """
        try:
            import ROOT
            from RDataFrameDSL import DSLCompiler
        ...
        try:
            ...
            # Use SEPARATE to_pandas() calls because full and sliced
            # columns have different per-event lengths (L1 constraint)
            df_full = dsl.to_pandas(rdf, ['full_pt', 'event_id'])
            df_sliced = dsl.to_pandas(rdf, ['sliced_pt', 'event_id'])
            
            # Compute per-event sums
            full_sums = df_full.groupby('event_id')['full_pt'].sum()
            sliced_sums = df_sliced.groupby('event_id')['sliced_pt'].sum()
