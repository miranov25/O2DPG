"""
Parametric benchmark: V5 sliding window.

Answers: How much does sliding window smoothing cost relative to
plain per-bin regression, when measured fairly?

Features:
  - Grid x Window x RPB parameter sweep
  - V1/V5/noSW backend comparison
  - 9 linear cost model fits with R² analysis
  - 6 consistency checks
  - TPC predictions (extrapolated)
  - Validation: V1 vs V5 coefficient diff (always on)
  - Optional cProfile profiling (--cprofile)

Usage:
    python bench_slidingwindow_parametric.py --quick     # 10 configs
    python bench_slidingwindow_parametric.py             # full scan
    python bench_slidingwindow_parametric.py --cprofile  # + per-config profiles
"""
import sys, time, os
import numpy as np
import pandas as pd
...
def profile_config(df, window, label):
    """Run cProfile on V5tot for one configuration, save .prof and .csv."""
    ...
    prof_file = os.path.join(PROFILE_DIR, f'{label}.prof')
    pr.dump_stats(prof_file)

    csv_file = os.path.join(PROFILE_DIR, f'{label}.csv')
    s = io.StringIO()
...
print("\n" + "="*80)
print("FAIR SMOOTHING OVERHEAD: V5arr vs noSW-arr")
print("="*80)
...
if DO_CPROFILE and profile_files:
    import zipfile
    zip_name = os.path.join(PROFILE_DIR, 'profiles_parametric.zip')
