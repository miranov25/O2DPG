...
**Current Phase:** 13.23c.GB-Audit (code work ✅ complete at commit `159ba256`; documentation sync this revision; `_END` tag pending)
**Document Version:** 7.1
**Drafted by:** Fable5_2GBAI (GBAI Coder) at architect request 2026-07-04 (v7.0 by Claude22, 2026-05-10)

> **v7.1 scope (2026-07-04).** This revision adds Phase 13.23c.GB-Audit: a one-day, 8-reviewer audit of the module at HEAD `c2d4fa55`/`6fe437c6` followed by a three-step fix set, all committed and reviewed the same day. Outcomes: bug-class instance #12 (`median_columns` silently ignored in v4) FIXED (commit `e3fdc9e0`, suffixed-name contract per architect ruling "Option B"); instance #9 (`boundary` ignored on the recompute path) WARNING-GUARDED with truth-in-metadata (commit `159ba256`, real fix still deferred); NEW instance #14 discovered (`kernel` ignored on the same path, ruling pending); instance #13 (`abs()`-sanitization naming) reclassified as not reproducible in GB source (pending architect confirmation, audit AD-Q6). `run_tests.sh` reviewer-packet integrity fixed (commit `97ad28b3`: unregistered-file warning, GB-first phase-tag anchor, debranding). Failure Modes Catalog expanded from 12 to 14 entries (#13 metadata misreport, #14 default-path coverage blind spot). Canonical baseline redefined: the "3 pre-existing failures" of v7.0 are in fact 2 stable failures plus a set of environment-sensitive performance flappers (`test_v3_numpy_faster_than_v1_numpy`, `test_evaluator_lookup::test_lookup_performance`) that flip within a single day on the same machine; both are proposed for exclusion from the functional gate. Baseline at close of 2026-07-04: **595 passed / 2 failed / 19 skipped** at `159ba256`.

> **v7.0 scope.** This revision adds four performance-optimization phases (13.20–13.23a) that landed between April 18 and May 10, 2026. Pipeline wall time reduced from 1452s to 722s (2×). CI roofline regression tests established (K=2.9 baseline, 10/10 pass). Profile-driven easy wins delivered (cProfile 61ms→45ms, -41% function calls). Parameter-not-propagated bug class updated from 9 to 11 instances (#10: calibration wrong-module import, #11: pytest venv-path divergence). Failure Modes Catalog expanded from 10 to 12 entries. Optimization roadmap planned (13.23b ADF, 13.24 dual-estimator, 13.25 BLAS kernel).
...
| **13.17.GB** | **`boundary='symmetric'` in `make_sliding_window_aggregate` — F1 fix + 16 invariance tests** | **Apr 9–11, 2026** | **✅ Complete (landed; basis of the aggregate-boundary behaviour referenced throughout; B5 correction v7.1)** |
| **13.19.GB-PERF** | **Fit path performance parity — dense-lookup replaces `_build_bin_index_map`** | **Apr 18, 2026** | **✅ Complete** |
| **13.20.GB-PERF** | **Numba-ize `_aggregate_window_dense` inner loop (CSR gather kernel)** | **Apr 18, 2026** | **✅ Complete** |
| **13.21.GB-PERF** | **Vectorize SW fit wrapper + prange gather + batch MAD (1452s→722s)** | **May 6, 2026** | **✅ Complete** |
| **13.22.GB-RooflineTier1** | **CI roofline regression tests (K=2.9, 10/10 pass, 12-cycle proposal)** | **May 9-10, 2026** | **✅ Complete** |
| **13.23a.GB-RooflineEasyWins** | **Profile-driven optimization: A1+A2+A3 (cProfile 61ms→45ms, -41% calls)** | **May 10, 2026** | **✅ Complete** |
| **13.17.GB-MedianFix** | **`agg_median=True` + `boundary` interaction** (D1 deferral from 13.17.GB per architect direction 2026-04-09) | **TBD** | **📋 Scheduled** |
| **13.23b.ADF-RooflineTier1** | **ADF team applies GB roofline template** | **TBD** | **📋 Proposed** |
| **13.24.GB** | **Dual-estimator (K_roofline + K_real) + ccsub0001 baseline** | **TBD** | **📋 Proposed** |
| **13.23c.GB-Audit** | **Module audit (8 reviewers) + fix set: bug #12 fixed, bug #9 warning, run_tests.sh integrity** | **Jul 4, 2026** | **✅ Code complete (`e3fdc9e0`, `97ad28b3`, `159ba256`); docs = this revision; tag pending** |
| **13.25.GB-BLASKernel** | **BLAS-batched OLS in fit_groups_single_numba** | **TBD** | **📋 Proposed** |
...
**Current test count (canonical `alma2`, 2026-07-04, commit `159ba256`, `SUMMARY_20260704_173802`):** **595 passed / 2 failed / 19 skipped**. The two failures are the stable deferred pair (`test_multiple_fits_match_v4_merged`, `test_tpc_distortion_recovery`). Two further tests are **flappers** — wall-clock performance assertions that flip pass/fail between runs on the same machine (`test_v3_numpy_faster_than_v1_numpy`; `test_evaluator_lookup::test_lookup_performance`, observed failing at 1.24× vs required 1.5× in `SUMMARY_20260704_134451` under 12-worker contention, passing 6 minutes later) — proposed for exclusion from the functional gate. Historical v7.0 line for reference: 575 passed / 3 failed / 19 skipped at Phase 13.23a.

> **Known documentation defect carried from FIX2 commit:** `TECHNICAL_SUMMARY.md` v3.3 records the canonical passed-test count as **528** at four locations (header summary, Current State table, arithmetic narrative, v3.3 document history row). The correct canonical number is **529** — the FIX2 commit flips `test_select_backend_auto_sequential` from failed to passed (F5 cleanup), which adds `+1` to the passed column that the Coder's arithmetic `517 + 11 = 528` omitted. Three reviewers flagged this during the FIX2 review cycle; the correction was not absorbed before commit. Closure is scheduled as part of TECHNICAL_SUMMARY v3.4 in Phase 13.17.GB per proposal §7.3. **This v7.0 uses 575 as the authoritative baseline.**

**Capability Matrix:** header regeneration pending (audit E3/B2 — regenerate with `--phase 13.23c.GB`; the committed matrix still carries a stale hardcoded phase header). Test state at v7.1: 595 passed / 2 stable failures / 19 skipped at `159ba256`; 0 broken features. The "575 passed / 3 pre-existing failures" figures elsewhere in this document are the historical Phase 13.23a values.
...
**Status update (Phase 13.23c.GB, 2026-07-04, commit `159ba256`):** the silent drop is now **warning-guarded**. `make_sliding_window_fit` emits a loud `UserWarning` ("boundary=... is IGNORED by algorithm='recompute' ...") whenever the default algorithm is combined with any non-`'full'` boundary dimension — architect ruling verbatim: *"OK. Let's make it a warning."* Computation is deliberately unchanged (bit-identity locked by test W5 in `tests/test_bug9_boundary_warning.py`). Additionally, returned metadata now records the boundary **actually used** (`'full'` on the recompute path) instead of the requested one — see Failure Mode #13. The parallel entry point is exempt: `make_sliding_window_fit_parallel` has no `algorithm` parameter and always routes through the V5 path, which honours `boundary` (verified: `_worker_v5` → `make_sliding_window_fit_v5_arrays(boundary=...)`, pristine-source lines :3975–3981 and :2721–2732). Production impact quantified during the audit: ~10.5–27% of bins in the `makeIterationFit123` I1–I3 grids are edge bins that received full-clipped instead of symmetric windows. **The real fix (recompute path honouring boundary) remains deferred** to Phase 13.XX.GB-BoundaryV1V2. Root cause of the bug's long invisibility is Failure Mode #14: every pre-existing boundary test passed `algorithm='incremental'` explicitly, so the default path was never exercised (audit Q7 finding, Sonnet5_2).
...
### Phase 13.20.GB-PERF: Numba-ize `_aggregate_window_dense` Inner Loop (Apr 18, 2026)

**Commit:** `1f3b45b3`
**Coder:** Claude22

Replaces the per-bin Python loop in `_aggregate_window_dense` with a JIT-compiled two-pass CSR kernel (`_get_gather_window_rows_kernel`):

- **F1:** Two-pass kernel (count → prefix sum → fill) replaces per-bin Python gather loop
- **F1a:** Removed redundant `flat_indices.clip` (16s — bounds mask suffices)
- **F1c:** Removed `np.unique(concatenate)` (37s — rows from different bins are disjoint by counting-sort guarantee; concatenation alone suffices)

Stats computation factored into `_compute_window_stats` (shared by both numba and numpy paths). Numpy fallback retained via `GBAI_DISABLE_AGG_DENSE_NUMBA=1` env flag.

**Tests:** 6 new invariance tests (numba kernel vs numpy fallback, parametrized window/fit_intercept/weights). Total: 20 pytest items, all passed on alma2.

---

### Phase 13.21.GB-PERF: Vectorize SW Fit Wrapper + Batch MAD (May 6, 2026)

**Commits:** `eea1046c` (Turn 1: F1+F2), `3f3bce25` (Turn 2: F3)
**Coder:** Claude22

Two-turn performance phase targeting the SW fit wrapper overhead and V4 median dispatch:

**Turn 1 — F1: `_fit_window_regression_numba` rewrite** — per-bin Python loops eliminated (47.8s self-time → vectorized):
- Pre-kernel: one `np.column_stack` for all bins; kernel handles NaN via `INVALID_FILTER` (was per-bin `np.isfinite` + `np.sum` + `np.column_stack` ≈ 30s)
- Post-kernel: vectorized R² from kernel `out_sum_y`/`out_sum_y2` (was per-bin `np.mean` + `np.sum` ≈ 18s)
- F1-kernel: `fit_groups_single_numba` extended with `out_sum_y`, `out_sum_y2` outputs (sufficient statistics for R², accumulated in existing prange bin loop at zero extra cost)

**Turn 1 — F2: `_gather_rows` prange parallelization** — `parallel=True` + `nb.prange` on Pass 1 (count) and Pass 2 (fill). Prefix sum stays sequential.

**Turn 2 — F3: Batch MAD in `make_parallel_fit_v4`** — `_get_batch_mad_kernel` replaces 1.26M per-bin `np.median` calls (52s cumulative) with one numba prange kernel call. Per-bin residuals stored in flat CSR array during OLS loop, batch MAD computed post-loop. Env flag `GBAI_DISABLE_BATCH_MEDIAN=1` for fallback.

**Production pipeline wall time:** 1452s → 722s (2×) with ~430s projected after remaining optimizations.

**Tests:** 575 passed, 3 pre-existing failures, 0 new regressions.

---

### Phase 13.22.GB-RooflineTier1: CI Roofline Regression Tests (May 9-10, 2026)

**Commits:** `28d828fe` (v1.12 initial), `15f4338e` (final with fixes)
**Proposal:** v1.12 (12 revision cycles, 4-reviewer panel approved)
**Coder:** Claude22

CI-integrated roofline performance metric measuring distance from ideal compiled code:

```
K = T_observed / T_expected,  where T_expected = Σ(n_ops × t_primitive)
```

K=1 means at hardware roofline. K>1 quantifies the optimization gap. Primitives from phase 12.12b Appendix A (M1/M2/M3w/C1/C2/C6), re-measured on target machine at test fixture working-set size.

**Results (alma2, 10/10 pass):**

| Test | K | T_obs | T_exp |
|---|---|---|---|
| SW fit pipeline (`make_sliding_window_fit`) | 2.9 | 42ms | 15ms |
| V4 pipeline (`make_parallel_fit_v4`, PyArrow) | 2.5 | 63ms | 25ms |
| Bin-id assignment (`_assign_bin_ids_fast`) | 2.3 | 1.1ms | 0.5ms |
| Fit kernel (OLS, C1+C2 denominator) | 3.3 | 41ms | 13ms |
| Batch MAD dispatch | 0 np.median calls | — | — |

**Deliverables:** D1 test file (10 tests: 4 K-roofline + 1 dispatch + 5 meta), D4/D5 calibration scripts (`measure_primitives.py`, `calibrate_roofline_K.py`, `update_roofline_baseline.py`), D7 documentation, D8 `run_tests.sh` gating (`WITH_ROOFLINE=1`).

**Key design decisions (from 12 proposal revisions):**
- Sum-of-primitives denominator (v1.3) — K measures distance from roofline, not arbitrary ratio
- M3w for output assembly, not P_pandas (v1.7) — denominator must be ideal compiled code
- Separate SW2D and S2 fixtures (v1.8) — SW fit needs 2D grid; V4 uses flat 1D
- C1 at window-sized operands, 2200 rows (v1.9) — SW fit OLS operates on window contents
- `backend='pyarrow'` pinned for V4 (v1.10) — model assumes PyArrow sort path
- Threading meta-test (v1.11) — calibration/test must match `NUMBA_THREADING_LAYER`
- Module-path assertion in calibration (v1.12) — prevents wrong-module import

**`[FOUND-WHILE-IMPLEMENTING]` Parameter-not-propagated bug class instance #10:** `backend='numba'` with `algorithm='recompute'` silently executes numpy path when imported via `sys.path.insert` (calibration script). `_resolved_backend` not correctly resolved at line 3538. Caught by Sonnet29 (fresh reviewer) via diagnostic snippet. Module-path assertion added as durable guard.

**`[FOUND-WHILE-TESTING]` Parameter-not-propagated bug class instance #11:** Pytest imports `groupby_regression_sliding_window.py` from venv editable install at `/Users/miranov25/venvlinux/venv310/src/o2dpg-utils/`, not from working copy. Previously masked by numba cache. Exposed by numba cache clearing during roofline validation.

**Counting-sort test removed:** cProfile unreliable on numba `@njit` kernel (same issue as v1.8 gather test removal per Sonnet27 P1-4). Timer noise at 0.19ms T_expected dominates.

**Threshold formula:** `K_threshold = K_floor + 6 × 1.4826 × MAD(K)` (architect-directed, robust statistics).

**Review panel:** Clude24 (continuity), Sonnet26, Sonnet27, Sonnet28, Sonnet29 (fresh — caught bug #10), Claude25 (drafter, conflict-of-interest declared), GPT1(GB). Strongest panel coverage of the phase.

**Tests:** 575 passed, 3 pre-existing failures, 10/10 roofline, 0 broken features in capability matrix.

---

### Phase 13.23a.GB-RooflineEasyWins: Profile-Driven Optimization (May 10, 2026)

**Commits:** `190c8e3f` (A1+A2), `e68c5b32` (A1+A2+A3)
**Proposal:** v0.2 (1 panel cycle, 5 reviewers, all APPROVE — fastest cycle in GBAI archive)
**Coder:** Claude22

Three local refactors guided by Phase 13.22 roofline profile (`bench_out/profile_sw_fit.prof`):

- **A1: Module-level Dispatcher cache** (`swf.py:688`, `swf.py:3669`) — cache `_get_gather_window_rows_kernel` and `_counting_sort_indices_numba` at module level. Pattern: `_get_numba_v4_kernels()` at line 2140. Eliminates per-call numba Dispatcher recreation.
- **A2: Listcomp vectorization** (`swf.py:1328`) — pre-stack `pred_matrix = np.column_stack(pred_arrays)` once, replace per-call listcomp with single 2D fancy-index `pred_matrix[all_idx]`.
- **A3: V5-style preallocated `_assemble_results`** (`swf.py:2881`) — replace per-bin dict construction (1024 × dictcomp with f-string keys) with pre-allocated numpy arrays + single `pd.DataFrame(data)` call. Eliminates pandas `maybe_convert_objects` overhead.

**Profile trajectory (cProfile, alma2):**

| Metric | Original | +A1+A2 | +A1+A2+A3 |
|---|---|---|---|
| cProfile total | 61ms | 56ms | **45ms (-26%)** |
| Function calls | 63,138 | 41,201 | **36,995 (-41%)** |

**Steady-state wall time:** 42ms → 42ms (unchanged). The cProfile-visible gains are real overhead elimination (22K fewer function calls, pandas dtype inference removed) but below wall-time measurement resolution at this fixture size. K remains 2.9–3.0.

**K=2.9 is structural:** OLS kernel (45%) + Python orchestration (36%). Closing the gap requires BLAS-batched OLS (Phase 13.25) and Arrow output (future).

**K ≤ 2.2 acceptance gate NOT MET.** Documented: the easy wins reduced cProfile overhead, not wall-time. The metric correctly identified that the remaining gap is structural.

**Tests:** 575 passed, 3 pre-existing failures, 10/10 roofline, 0 new regressions.

---

## July 2026 Phases

### Phase 13.23c.GB-Audit (2026-07-04) — Audit + same-day fix set

**One-day audit of `groupby_regression/` at HEAD `c2d4fa55` (working baseline re-verified at `6fe437c6`), followed by three reviewed, committed fix steps.** Audit questions: `AUDIT_GB_2026_07 v1.1` (author Fable5_1GBAI, Main Reviewer). Panel: 8 reviewer inputs (Fable5_2GBAI coder, Sonnet27, Sonnet28, Sonnet29, Sonnet5_1GBAI, Sonnet5_2GBAI, GPT1:GBAI ×3, Clude24), synthesized in `AUDIT_GB_2026_07_v1.0_Consolidated_Audit_Summary`. Sonnet29 rotated out after source-refuted claims (Q1 ranking).

**Key audit findings (evidence anchors in the consolidated summary):**
- **P1-1 / instance #9 confirmed with aggravator:** recompute path ignores `boundary` AND the returned metadata recorded the *requested* (unapplied) mode — self-certifying silent miscomputation. ~12 production call sites (`makeIterationFit123`).
- **P1-2 / instance #12 confirmed:** `make_parallel_fit_v4(median_columns=...)` accepted, validated, silently unused; **three** production call sites, two carrying manual-workaround merges; metadata listed median columns that never existed.
- **NEW instance #14:** `kernel`/`kernel_width` silently ignored on the same recompute branch, same metadata misreport (no known production impact — all observed production calls use `kernel='uniform'`). **Ruling pending.**
- **Instance #13 reclassified:** the `aqpt_ITSTPC` abs()-sanitization naming issue is not reproducible in GB source — the name is user-chosen in a production alias, not GB auto-generation (pending architect confirmation, AD-Q6).
- **Layer B / C++ commissioning blocker:** `Makefile` references `gbe_root_io.h/.cxx` and the invariance test requires `dfGB_to_root.py`; none of the three exist in versioned source (AD-Q3, open).
- **Tooling integrity (E1/E2):** `run_tests.sh` diff anchor searched only dfdraw/ADF tags; banner said "dfdraw Test Runner".
- **Baseline redefinition:** single-run baselines are insufficient — two performance assertions flap within one day on the same machine. Honest baseline form: *2 stable failures + 0–2 flappers*.

**Step 1 — bug #12 fix (commit `e3fdc9e0`).** `make_parallel_fit_v4` now computes per-group medians over the post-`selection` rows (NaN skipped within each group), output column **`f"{col}{suffix}"`**. Architect rulings verbatim: *"D2.) It is bug and should be fixed. We should make bug report"* and, on naming, *"Option B"* / commit-recorded *"+AD We should use suffixes."* — true legacy parity for all suffixes (`groupby_regression.py:88-89` compute, `:538` suffix rename). ValueError on name collision; metadata lists the actual output names. Tests: `tests/test_bug12_median_columns_v4.py` (12: exact groupby-oracle incl. NaN + float16 columns, row-set discrimination, legacy cross-fitter parity for empty and non-empty suffix, 2 collision guards, no-op tri-identity + column-stability bit-exact vs pristine module, metadata truth). Real-data verification by architect: `calibVertex` medians ≡ manual workaround (`np.allclose == True`) on 9.86M tracks. Canonical: 588P/2F at bundle `145909` (the intermediate bundle `145353` showed 586P/4F due to a disclosed file-sync defect — stale interim test file on alma2 — and is not a clean data point). Residual caller action: `calibVertex` (suffix="") workaround lines `time_series.py:642–643` must be deleted (suffixed naming rescues the two `makeSmoothMapsWithTPC` sites but not the empty-suffix caller).

**Step 2 — run_tests.sh reviewer-packet integrity (commit `97ad28b3`).** Architect ruling verbatim (typos preserved): *"We will need to path run_test.sh in the same way as in the dfdraw and ADF - where I got a warning if the test is not registered"*. (a) Untracked `*.py` files now produce a loud SUMMARY warning and their full content is appended to both diff artifacts (`git diff --no-index`, no index mutation; `-uall` so files in new untracked directories are not collapse-hidden) — non-blocking, unlike the dfdraw exit-1 original. v2 of the fix was required same-day: the canonical bundle `152916` proved v1 silently skipped content because porcelain paths are repo-root-relative (prefix-strip added; honest note: the coder's first synthetic harness could not have caught this — the canonical run did). (b) Phase-diff anchor now searches GB tags first (`PHASE_BEGIN_groupby*`, newest `PHASE_13_*_GB_*`), dfdraw/ADF only as fallback. (c) Debranding. First canonical run with v2 surfaced `tutorials/thn_k0s_fit.py`, previously hidden by directory collapsing. Partial `.gitignore` for build/bench artifacts in the same commit (audit AD-Q4, remainder open). Mode-bit note: the file transfer stripped `+x` (commit shows `100755 => 100644`); restored in a follow-up — delivered shell scripts must state "restore +x after copy".

**Step 3 — bug #9 warning + metadata truth (commit `159ba256`).** Described in the Incident 9 status update above. Tests: `tests/test_bug9_boundary_warning.py` (7). The pre-existing characterization test `test_v1v2_boundary_parameter_silently_dropped` was deliberately NOT modified (work-order rule); it passes on canonical alma2 with the new warning visible in the log. A sandbox failure of that test was bisect-proven to be a pandas-3 environment artifact (pristine module fails identically).

**Parameter-not-propagated bug class: 11 → 14 instances.**

| Instance | What | Status (2026-07-04) |
|---|---|---|
| #12 | `median_columns` silently ignored in `make_parallel_fit_v4` | ✅ **FIXED** commit `e3fdc9e0` |
| #13 | abs()-sanitization alias naming (`aqpt_ITSTPC`) | ♻️ **Reclassified**: user-chosen name in production script, not GB code (AD-Q6, pending architect confirm) |
| #14 | `kernel`/`kernel_width` ignored on recompute path (metadata misreports) | ⚠️ **OPEN** — warning ruling pending (recommended: identical one-liner to #9) |

**Canonical baselines of the day (all alma2, 12 workers):** `134451`: 574P/4F (both flappers down) → `135025`: 586P/3F (step-1 interim) → `145909`: 588P/2F (step 1 final) → `154310`: 587P/3F (step 2) → `173802`: **595P/2F/19S at `159ba256`** (step 3; both flappers up). Arithmetic closes at every step.

**Open items at v7.1:** `_END` tag after this doc sync; #14 ruling; AD-Q3 Layer B disposition; AD-Q4 untracked remainder (`bench_sliding_window.py` move, `benchmarks/` keepers `git add`); AD-Q8 stale-test deletion (`test_aggregate_numba_matches_numpy`); B3 matrix `?` cells (nonlinear column); calibVertex workaround deletion; roofline `WITH_ROOFLINE=1` re-baseline at new HEAD.
...
At the time of the P0 `fit_intercept` fix (Mar 29, 2026), the parameter-not-propagated bug class had 3 known instances. In the six weeks since, two more have been found — F2 (`method=dict` in FIX2) and F1 (`boundary` in the aggregate path, in-flight in 13.17.GB). By Phase 13.22 (May 2026), the class instance count reached **11** — including `fit_intercept` at multiple locations (#6–#8), `boundary='symmetric'` in aggregate path (#9), calibration wrong-module import (#10), and pytest venv-path divergence (#11). The recurring pattern: every code path × backend combination that isn't explicitly tested can silently drop a parameter.
...
### Capability Matrix (Phase 13.23a.GB, canonical `alma2`)
...
| Partial (⚠️) | 0 (0.0%) |
| Planned (📋) | 1 (0.8%) |
| Total tests passed (canonical) | **575** |
| Pre-existing failures | **3** |
| Skipped | **19** |
| Roofline tests (opt-in) | **10/10** |
| Parameter-not-propagated instances | **11** |
...
| **11** | **Calibration imports wrong module via `sys.path.insert`** | **Phase 13.22 calibration script imported stale system install instead of live source — `_resolved_backend` bug caused numpy fallback, K_cal=15 vs K_test=2.9. Caught by Sonnet29 fresh reviewer. Parameter-not-propagated instance #10** | **Module-path assertion at calibration startup: `assert os.path.abspath(_sw.__file__) == os.path.abspath("groupby_regression_sliding_window.py")`** |
| **12** | **Pytest imports from different file than working copy (venv editable install)** | **Phase 13.23a: pytest loaded module from `/venvlinux/venv310/src/o2dpg-utils/` not from working copy. Masked by numba cache; exposed by cache clearing. Parameter-not-propagated instance #11** | **Symlink venv install to working copy, or copy after edits. Future: single-source editable install** |
| **13** | **Metadata records requested-not-applied parameters** (output self-certifies behaviour that did not happen) | **Audit 2026-07-04, three instances: SW recompute metadata claimed the requested `boundary_mode` while computing 'full' (sliding_window pristine :3659); v4 metadata listed suffixed median columns that were never computed (optimized pristine :485–486); same-branch `kernel` metadata (pristine :3663–3664)** | **Rule: metadata fields must be written from the values consumed by the executed path, not from raw arguments. Locked for #9 by test W4 (`test_bug9_boundary_warning.py`); for #12 by `test_metadata_medians_lists_actual_output_names`. Kernel instance pending #14 ruling** |
| **14** | **Coverage blind spot: every test of a feature pins the non-default code path** | **Audit 2026-07-04 (Sonnet5_2): all pre-existing `boundary` tests passed `algorithm='incremental'` explicitly — the default recompute path was never exercised, which is why instance #9 stayed invisible for months while fully "tested"** | **Invariance tests must parametrize over {default (argument omitted), each explicit path}; reviewer checklist item: "is the DEFAULT dispatch exercised?"** |
...
|-------|---------|--------|
| **13.23b.ADF-RooflineTier1** | **ADF team applies GB roofline template — separate primitives (M-disk-write, M-compress, M-join), own variance characterization** | **📋 Proposed** |
| **13.24.GB-OptimizationRoadmap** | **`_aggregate_window_dense` V5-style vectorization (~2ms), dual-estimator K_roofline + K_real, ccsub0001 baseline** | **📋 Proposed** |
| **13.25.GB-BLASKernel** | **BLAS-batched OLS in `fit_groups_single_numba` — ceiling ~halving kernel time (15ms → ~7ms)** | **📋 Proposed** |
| 13.17.GB-MedianFix | `agg_median=True` + `boundary` interaction — median path honors mask | 📋 Scheduled |
...
| — | Symlink venv editable install to working copy (dual-path bug fix) | Micro-task |
| — | Delete pre-existing broken `test_aggregate_numba_matches_numpy` (superseded by T14) | Separate micro-task; deletion proposal filed in audit (AD-Q8), architect approval pending |
| — | #14: `kernel` warning on recompute path (twin of #9) | Awaiting architect ruling; recommended identical one-liner |
| 13.XX.GB-BoundaryV1V2 | Real fix: recompute path honours `boundary` (now warning-guarded) | 📋 Deferred, unchanged |
| — | Flapper gate exclusion: move `test_v3_numpy_faster_than_v1_numpy` + `test_lookup_performance` out of functional gate (perf marker or serial run) | Audit proposal, pending |
| — | AD-Q4 remainder: `bench_sliding_window.py` → `benchmarks/`, `git add` the two untracked `benchmarks/` keepers, duplicate `cpp/include/json_reader.hpp` disposition | Pending, tracked live by the run_tests.sh unregistered-file warning |
...
| **7.0** | **May 10, 2026** | **Phases 13.20–13.23a landed.** Performance optimization sequence: Phase 13.20 (numba-ize `_aggregate_window_dense`, commit `1f3b45b3`), Phase 13.21 (vectorize SW fit wrapper + prange gather + batch MAD, commits `eea1046c` + `3f3bce25`, pipeline 1452s→722s), Phase 13.22 (CI roofline regression tests, K=2.9, 12-cycle proposal, commit `15f4338e`), Phase 13.23a (profile-driven easy wins A1+A2+A3, cProfile 61ms→45ms, commit `e68c5b32`). **Parameter-not-propagated bug class updated** from 9 to 11 instances: #10 calibration imports wrong module via `sys.path.insert` (Sonnet29), #11 pytest imports from venv editable install not working copy. **Failure Modes Catalog expanded** from 10 to 12 entries: #11 wrong-module import via sys.path, #12 venv/working-copy path divergence. **Capability Matrix updated** to 575 tests passed, 0 broken features, 10/10 roofline. **Planned Phases updated** with roofline-driven optimization roadmap (13.23b ADF, 13.24 dual-estimator, 13.25 BLAS kernel). **Drafted by Claude22 (GBAI Coder) at architect request 2026-05-10.** |
| **7.1** | **Jul 4, 2026** | **Phase 13.23c.GB-Audit landed** (audit + 3-step fix set, commits `e3fdc9e0`/`97ad28b3`/`159ba256`). Bug-class instance #12 FIXED (v4 medians, suffixed contract), #9 WARNING-GUARDED + metadata truth, #14 NEW (kernel, pending ruling), #13 reclassified (AD-Q6 pending). Failure Modes Catalog 12 → 14 (#13 metadata misreport, #14 default-path blind spot). Baseline redefined (stable pair + performance flappers); canonical at close: **595P/2F/19S** at `159ba256`. Incident 9 status updated. July 2026 Phases section added. Drafted by Fable5_2GBAI at architect request 2026-07-04. |
