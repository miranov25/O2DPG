...
## Behavior Changes (Phase 13.19.GB-PERF)

**Output row ordering:** `make_sliding_window_fit` with `algorithm='recompute'` (V1/V2, default) now produces output DataFrame rows sorted lexicographically by bin coordinates (row-major order via `np.unique`). Previously, rows were in data-encounter order (Python dict insertion order). Coefficient values are identical; only row ordering differs. Locked by `test_output_row_order_sorted_lex` in `test_fit_path_perf_invariance.py`.

Row positions of output are not stable across `algorithm=` values; they were not stable before this phase either. Consumers that compare `algorithm='recompute'` output to `algorithm='incremental'` output must merge by bin-coordinate key, not by row position. All ADF and O2DistAI pipeline joins use bin-coordinate keys and are unaffected.
